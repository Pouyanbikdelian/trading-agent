...
def load_prices(
    ...
    *,
    ...
    if dropped:
        print(
            f"  dropped {len(dropped)} symbols with < {min_history_frac:.0%} history: "
...
def main(argv: list[str]) -> int:
    ...
    for name, params in strategy_params.items():
        try:
            ...
            print(
                f"  {name:18s} CAGR={m['cagr']:>7.2%}  Sharpe={m['sharpe']:>5.2f}  "
    ...
    for combiner_name in ["equal_weight", "inverse_vol", "min_variance", "dsr_weighted"]:
        try:
            ...
            print(
                f"  {combiner_name:18s} CAGR={m['cagr']:>7.2%}  Sharpe={m['sharpe']:>5.2f}  "
                f"MaxDD={m['max_drawdown']:>7.2%}  Trades={int(m['n_trades']):>5d}"
            )
        except Exception as e:
    ...
    print(
        f"  {'#':>2} {'name':22s} {'CAGR':>8} {'Sharpe':>7} {'Sortino':>7} {'MaxDD':>8} {'Calmar':>7}"
    )
    for i, r in enumerate(leaderboard, 1):
        print(
            f"  {i:>2} {r['name']:22s} {r['cagr']:>8.2%} {r['sharpe']:>7.2f} "
