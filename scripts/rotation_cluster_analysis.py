...
print("=" * 72)
print("WINDOW", px.index[0].date(), "→", px.index[-1].date(), f"({len(px)} monthly obs)")
...
print("\n" + "=" * 72)
print("1) CLUSTER ANTI-CORRELATION  (monthly RELATIVE-to-SPY returns)")
print(f"   corr(DEF composite, RISK composite) = {Dr.corr(Rr):+.2f}")
print(
    f"   corr on ABSOLUTE returns            = {absr[DEF].mean(1).corr(absr[RISK].mean(1)):+.2f}  (beta not stripped)"
)
...
print(
    f"     DEF internal avg  = {np.mean([rel[a].corr(rel[b]) for a in DEF for b in DEF if a < b]):+.2f}"
)
print(
    f"     RISK internal avg = {np.mean([rel[a].corr(rel[b]) for a in RISK for b in RISK if a < b]):+.2f}"
)

print("\n" + "=" * 72)
print("2) LEAD-LAG  corr(DEF_rel(t), RISK_rel(t+k))  [k<0: RISK leads DEF]")
for k in range(-3, 4):
    print(f"     k={k:+d} : {Dr.corr(Rr.shift(-k)):+.2f}")

# --- RRG reproduction (monthly analogue of dashboard 63/21 daily) ---
print("\n" + "=" * 72)
print("3) RRG QUADRANTS  (RS-Ratio 6m-mean, RS-Momentum 3m; dashboard logic)")
...
Q = pd.DataFrame(
    {c: [quad(ratio[c].iloc[i], mom[c].iloc[i]) for i in range(len(px))] for c in THEMES}, index=idx
).iloc[8:]
...
opp = sum(
    ...
    if (side(row)[0] > 0 and side(row)[1] < 0) or (side(row)[0] < 0 and side(row)[1] > 0)
)
print(
    f"   months where clusters sit on OPPOSITE sides (one leading-tilt, other lagging-tilt): {opp}/{len(Q)} = {opp / len(Q) * 100:.0f}%"
)
print("   latest quadrants:")
print(Q.iloc[-1].to_string())
...
rot = pd.Series(
    [
        cluster_ret(RISK).iloc[i] if pos[i] == "RISK" else cluster_ret(DEF).iloc[i]
...
mn = pd.Series(
    [
        (cluster_ret(RISK).iloc[i] - cluster_ret(DEF).iloc[i])
        if sig.shift(1).iloc[i] > 0
        else (cluster_ret(DEF).iloc[i] - cluster_ret(RISK).iloc[i])
        for i in range(len(sig))
    ],
    index=sig.index,
).dropna()
...
def stats(r, label):
    ...
    shp = (r.mean() * ann) / (r.std() * np.sqrt(ann)) if r.std() > 0 else np.nan
    eq = (1 + r).cumprod()
    dd = (eq / eq.cummax() - 1).min()
    print(
        f"   {label:22s} CAGR {cagr * 100:6.1f}%  vol {vol * 100:5.1f}%  Sharpe {shp:5.2f}  maxDD {dd * 100:6.1f}%  hit {100 * (r > 0).mean():4.0f}%"
    )


print(
    f"   (n={len(rot.dropna())} months, signal flips to RISK {100 * (pd.Series(pos) == 'RISK').mean():.0f}% of months)"
)
stats(spy, "SPY (benchmark)")
stats(cluster_ret(DEF), "Always DEF")
stats(cluster_ret(RISK), "Always RISK")
stats(rot, "Rotation (long-only)")
stats(mn, "Long leader/short lag")

# --- Regime split ---
print("\n" + "=" * 72)
print("5) SUB-PERIOD corr(DEF_rel,RISK_rel)")
for lab, a, b in [
    ("2021-07..2022-12 (bear/infl)", "2021-07", "2022-12"),
    ("2023-01..2024-12 (AI bull)", "2023-01", "2024-12"),
    ("2025-01..2026-07 (recent)", "2025-01", "2026-07"),
]:
    m = (Dr.index >= a) & (Dr.index <= b)
    print(f"   {lab:30s} corr={Dr[m].corr(Rr[m]):+.2f}  n={m.sum()}")
print("\n" + "=" * 72)
print("6) PERSISTENCE  rolling 12m corr(DEF_rel, RISK_rel)")
rc = Dr.rolling(12).corr(Rr).dropna()
print(
    f"   mean {rc.mean():+.2f}  min {rc.min():+.2f}  max {rc.max():+.2f}  share of windows <0: {100 * (rc < 0).mean():.0f}%"
)
...
mn_mr = pd.Series(
    [
        ...
        if sig.shift(1).iloc[i] > 0
        else (cluster_ret(RISK).iloc[i] - cluster_ret(DEF).iloc[i])
        for i in range(len(sig))
    ],
    index=sig.index,
).dropna()
...
print(
    f"   3m rel-momentum spread (RISK-DEF) = {sig.iloc[-1]:+.3f}  -> momentum favors {'RISK' if sig.iloc[-1] > 0 else 'DEF'}"
)
print(f"   latest 1m relative move: DEF {Dr.iloc[-1] * 100:+.1f}%  RISK {Rr.iloc[-1] * 100:+.1f}%")
print(
    f"   RRG tilt now: DEF leading-count {sum(Q.iloc[-1][c] == 'leading' for c in DEF)}/3, RISK leading-count {sum(Q.iloc[-1][c] == 'leading' for c in RISK)}/3"
)
print("\nDONE")
