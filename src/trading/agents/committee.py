...
MANAGER_CHARTER = (
    ...
    "Their `retrieval_role` is deliberate: apply a `regime_match` as an "
    "active check; treat a `partial_match` with its listed differences as "
    "a hypothesis; and use a `broad_prior` or `diversifier` to test for "
    "anchoring rather than assume its regime is active. If you rule against "
    "one, say which and why, because a "
